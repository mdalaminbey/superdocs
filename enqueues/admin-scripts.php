...
wp_enqueue_style( 'superdocs-tailwind', $application->get_root_url() . 'assets/css/app.css', [], time() );

if ( wp_commander_is_admin_page( 'edit', ['post_type' => superdocs_template_post_type()] ) ) {
    wp_enqueue_script( 'superdocs-template-js-' . $application::$config['namespace'] . '-defer', $application->get_root_url() . '/assets/js/admin/template.js', ['jquery'], time(), true );
}
