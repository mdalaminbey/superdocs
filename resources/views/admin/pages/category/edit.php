...
<div class="p-7" x-data="<?php echo $dataKey ?>">
  ...
  <div class="mt-4">
    <form action="" method="post" @submit.prevent="updateCategory($data)">
      ...
      <button class="btn mt-2 cursor-pointer rounded-md bg-success py-2 px-8 font-semibold text-white hover:bg-success-hover focus:bg-success-hover active:bg-success-hover/90"><?php esc_html_e('Sumbit', 'wp-guide')?></button>
    </form>
  </div>
</div>
<script>
	Alpine.data('<?php wp_commander_render( $dataKey )?>', () => ({
		updateCategory($data) {
			...
			jQuery.ajax({
				url: "<?php wp_commander_render( get_rest_url( null, 'wp-guide/category/update' ) )?>",
				method: 'POST',
