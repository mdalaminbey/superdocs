...
<div class="p-7" x-data="<?php wp_commander_render($dataKey) ?>">
  <h2 class="line-clamp-1 dark:text-navy-100 text-xl font-bold tracking-wide text-slate-700 lg:text-base"><?php esc_html_e('Edit Category', 'super-docs')?></h2>
  <hr class="mt-2" />
  <div class="mt-4">
    <form action="" method="post" @submit.prevent="updateCategory($data)">
      ...
      <button class="btn mt-2 cursor-pointer rounded-md bg-success py-2 px-8 font-semibold text-white hover:bg-success-hover focus:bg-success-hover active:bg-success-hover/90"><?php esc_html_e('Sumbit', 'super-docs')?></button>
    </form>
  </div>
</div>
<script>
	Alpine.data('<?php wp_commander_render( $dataKey )?>', () => ({
		updateCategory($data) {
			...
			jQuery.ajax({
				url: "<?php wp_commander_render( get_rest_url( null, 'super-docs/category/update' ) )?>",
				method: 'POST',
